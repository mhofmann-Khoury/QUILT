...
class TestCourse_Merge_Process(TestCase):
    ...
    def test_merge_jersey_lace(self):
        connection = self._make_connection('jersey', 'lace', c=1, width=6, height=2)
        merger = Course_Merge_Process(connection)
        merger.merge_swatches()
    ...
    def test_merge_jacquard(self):
        ...
        connection = self._make_connection('jacquard', 'jacquard', white=1, black=2, width=5, height=3)
        merger = Course_Merge_Process(connection)
        merger.merge_swatches()
        merger.compile_to_dat('jacquard_merge')
    ...
    def test_merge_seed_jacquard(self):
        ...
        connection = self._make_connection('jacquard', 'seed', c=1, white=1, black=2, width=4, height=2)
        merger = Course_Merge_Process(connection)
        merger.merge_swatches()
