...
def cleanup_test_files(extensions: list[str] | None = None, preserve_set: set[str] | None = None) -> int:
    ...
    Args:
        extensions (list[str] | None, optional): List of file extensions to delete. Defaults to ['.k', '.dat'].
        preserve_set (set[str] | None, optional): Set of filenames to preserve even if they match extensions. Defaults to an empty set which will preserve no files.
